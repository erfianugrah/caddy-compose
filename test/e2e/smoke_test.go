...
func TestPolicyEngineDetectMatchDetails_PhraseMatch(t *testing.T) {
	...
	if found == nil {
		t.Fatal("detect_block event with e2e-scanner-beta UA not found")
	}
...
func TestPolicyBlockEvent_RequestContext(t *testing.T) {
	...
	// Step 3: Wait for log tailing and check events API.
	time.Sleep(12 * time.Second)

	_, eventsBody := httpGet(t, wafctlURL+"/api/events?hours=1&limit=50&event_type=policy_block")
	...
	if found == nil {
		t.Fatal("could not find the triggered policy_block event with sentinel UA")
	}
