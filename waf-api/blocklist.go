...
func (bs *BlocklistStore) Refresh(deployCfg DeployConfig) BlocklistRefreshResponse {
	...
	// Generate Caddy-importable snippet (same format as update-ipsum.sh).
	now := time.Now().UTC().Format(time.RFC3339)
	ipLine := strings.Join(ips, " ")
	content := fmt.Sprintf(`# AUTO-GENERATED by waf-api refresh — do not edit manually
# Updated: %s
# IPs: %d (min_score=%d)
# Source: %s
...
func handleBlocklistRefresh(bs *BlocklistStore, deployCfg DeployConfig) http.HandlerFunc {
	return func(w http.ResponseWriter, _ *http.Request) {
