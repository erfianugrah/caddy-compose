...
```bash
cd wafctl && CGO_ENABLED=0 go build -ldflags="-s -w -X main.version=0.22.0" -o wafctl .
```
...
Caddy tag format: `<project-version>-<caddy-version>` (e.g. `1.30.0-2.11.1`).
wafctl tag format: simple semver (e.g. `0.22.0`).
