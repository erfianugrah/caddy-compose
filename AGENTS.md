...
```bash
cd wafctl && CGO_ENABLED=0 go build -ldflags="-s -w -X main.version=2.102.0" -o wafctl .
```
...
- Composer (`composer.erfi.io`) decrypts `.env` at deploy time using its loaded age key (`COMPOSER_SOPS_AGE_KEY` env var). The age recipient in `.env`'s SOPS metadata MUST match composer's public key (`age132gmayefg7mq9t7fdfh9ppczn009uqyql49yje89fcjcp74v84aq36gu87` as of 2026-05-24).
...
- **A Caddyfile-only change needs `docker restart caddy` after `make restart`** (learned 2026-09-05). The Caddyfile is a single-file bind mount; composer's git sync replaces the file with a new inode and the running container keeps the old one, so `make restart` (service definition unchanged, no recreate) and `make caddy-quick-reload` (`config is unchanged`) both deploy nothing. Verify with `docker exec caddy grep -c <new-token> /etc/caddy/Caddyfile` vs the host copy, then `ssh router 'docker restart caddy'` (same container, SOPS env preserved; see "Caddy reload vs restart" below).
- **Snippet imports resolve TOP-DOWN**: `import <snippet>` must sit AFTER the `(snippet)` definition in `deploy/edge/Caddyfile` - a forward reference is treated as a file import and crash-loops Caddy (`File to import not found`). Most snippets are at the top, but `(lan_only)` / `(research_auth)` / `(memledger_auth)` are defined mid-file (~line 820), so site blocks using them must live below that point (learned the crash-loop way 2026-08-11, ha/esphome.erfi.io).
- **Static SPA behind `handle_path /ui*`: don't use bare `caddy file-server` in the upstream.** A directory request (`/ui/sessions` -> stripped to `/sessions`) makes the file-server 308 to `/sessions/`, which escapes the `handle_path` block and falls through to the next handler (404). Fix: upstream serves `try_files {path}/index.html {path} /index.html` (memledger `web/Caddyfile`, 2026-08-29). Related: the browser probes bare `/favicon.ico` before parsing the HTML, so if the page's linked icon is under `/ui/`, the edge needs an explicit `/favicon.ico` handler too.
...
- **Edge HTTP cache** (`caddyserver/cache-handler` Souin core + `darkweak/storages/nuts` on-disk storage, pinned in `Dockerfile`). **REMOVED from the running config 2026-08-07 (commit 02b0706)** - the souin fork + nuts modules are still compiled into the Caddy image, but no `cache` handler is configured in `deploy/edge/Caddyfile`. Everything below in this bullet is the pre-removal behavior, kept as the reference for re-enabling; the canonical quirk list with evidence lives in `test/cache/README.md`. Note `tools/cachectl verify` asserts a global cache block that no longer exists. What it was: a global `cache {}` block set defaults (`ttl 60s`, `stale 1h`, `default_cache_control no-store`); caching activated per-site via the `cache` handler. Sites that had it: `docs.erfi.io` (whole-site, 5m fresh / 24h keep, `disable_query`), `jellyfin.erfi.io` (route-scoped to `/Items/*/Images/*` only, 24h fresh / 72h keep; media streams/API untouched), `navidrome.erfi.io` (route-scoped to `/rest/getCoverArt*`, 24h fresh / 72h keep; Subsonic auth params in query -> per-client keys), and `erfianugrah.com` + `revista.erfi.io` (whole-site static Astro, TTL governed by response Cache-Control; `disable_query`). Ordering was `order cache after policy_engine` (WAF inspected every request; blocks never entered the cache). Behavioral quirks verified by the local harness in `test/cache/` (`make test-cache`, full evidence + source refs in `test/cache/README.md`):
  ...
  - **`disable_query` controls cache-key cardinality on docs.erfi.io** (`deploy/edge/Caddyfile`, flat `key { disable_query }` inside the site `cache {}` block - NOT the regex `cache_keys{}` mechanism). Whole-site + fully public + no upstream query validation means arbitrary `?spam=N` strings would otherwise mint unbounded distinct entries per path (a no-auth cache-blowing/amplification vector). Jellyfin deliberately keeps the query in the key (`api_key` lives there). Verified in test/cache suite section 16.
  - **Per-site nuts storage is REQUIRED (2026-07-31 incident).** Every cache-enabled site block carries its own `nuts { configuration { Dir /data/cache/nuts/<site>; EntryIdxMode HintKeyAndRAMIdxMode } }` - NEVER a global `nuts` block (provider inheritance makes every handler race to open its own nutsdb on the same path; losers silently fall back to in-memory, winners are multi-writers on one dir), and `Dir` MUST sit inside `configuration` (`nuts.Factory` drops `provider.Path` when `Configuration` is non-nil -> silent relocation to container-tmpfs `/tmp/souin-nuts`, cache wiped per restart). Keys-only RAM index means container RAM no longer scales with cached bytes. No size bound or LRU exists; bounds are ttl+stale and `disable_query`. Ops: `tools/cachectl` (Go CLI, `cd tools/cachectl && go run . status|verify|probe|purge`; purge = the only working reclaim). Full detail: test/cache/README.md "Per-site storage" + "Bounded storage and eviction".
