...
```bash
cd wafctl && CGO_ENABLED=0 go build -ldflags="-s -w -X main.version=2.60.0" -o wafctl .
```
...
- **CRS metadata**: Category taxonomy, valid prefixes, and severity levels are loaded from
  ...
  Falls back to compiled-in defaults when file is missing. `crs_metadata.go` holds the loader,
  `atomic.Pointer[CRSMetadata]` for thread-safe access. All hardcoded category maps are removed;
  `normalizeCRSCategory()` and `IsValidPrefix()` read from the loaded metadata.
