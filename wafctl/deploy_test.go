...
func TestDeployEndToEnd_ExclusionAndSettings(t *testing.T) {
	...
	mux.HandleFunc("POST /api/config/deploy", handleDeploy(configStore, exclusionStore, rateLimitStore, NewManagedListStore(filepath.Join(t.TempDir(), "lists.json"), t.TempDir()), nil, nil, nil, deployCfg))
...
func TestDeployEndToEnd_CaddyReloadFails(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/api/config/deploy",
		nil)
	w := httptest.NewRecorder()
	handleDeploy(configStore, exclusionStore, rateLimitStore, NewManagedListStore(filepath.Join(t.TempDir(), "lists.json"), t.TempDir()), nil, nil, nil, deployCfg)(w, req)
