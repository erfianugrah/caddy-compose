...
func (bs *BlocklistStore) parseFile() {
	// Read the entire file at once — no line-length limits.
	// With min_score=1 the @ipsum_blocked line can be several MB (200k+ IPs);
	// bufio.Scanner's token limit would silently truncate it.
	data, err := os.ReadFile(bs.filePath)
	if err != nil {
		log.Printf("[blocklist] cannot read %s: %v", bs.filePath, err)
		return
	}

	// Stat the file for mtime fallback if "# Updated:" comment is missing.
	var fileMtime time.Time
	if info, err := os.Stat(bs.filePath); err == nil {
...
func (bs *BlocklistStore) Refresh(deployCfg DeployConfig) BlocklistRefreshResponse {
	...
	// Always use the compiled-in default — don't inherit the old file's
	// min_score, which would silently preserve a stale threshold after an
	// upgrade (e.g. old file says min_score=3 but we now want 1).
	minScore := defaultBlocklistMinScore
